...
export function UserDetailClient({ user }: UserDetailClientProps) {
  ...
  const watchedAccessMap = Object.fromEntries(
    ACCESS_FIELDS.map((field, index) => [
      field,
      Boolean(watchedAccessValues[index]),
    ]),
  ) as AccessMap;
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-zinc-600">
            Lihat data akun user dan kelola aksi admin.
          </p>
        </div>
      ...
      <div className="rounded-xl border border-zinc-200 bg-white p-5">
        ...
        <div className="mt-2 space-y-1 text-sm text-zinc-600">
          {!user.actorCanManage ? (
            ...
            <p>
              User ini adalah superadmin. Semua akses selalu aktif secara sistem
              walaupun nilai database berubah.
            </p>
          ) : (
            <p>
              Ubah hak akses user berdasarkan halaman dan tanggung jawab
              operasional.
            </p>
          )}
        </div>
        ...
        <Form {...accessForm}>
          ...
          >
            ...
            <Button
              type="submit"
              disabled={
                !user.canUpdateAccess || isBusy || !accessForm.formState.isDirty
              }
            >
              Simpan Hak Akses
            </Button>
      ...
      <div className="rounded-xl border border-zinc-200 bg-white p-5">
        ...
        <div className="mt-3 space-y-2 text-sm text-zinc-600">
          {!user.actorCanManage ? (
            <p>
              Hanya superadmin (user paling awal dibuat) yang dapat melakukan
              reset password atau hapus user.
            </p>
          ) : user.isSuperadmin ? (
            ...
            <Button
              ...
              onClick={() => {
                const isConfirmed = window.confirm(
                  `Reset password user \"${user.name}\" ke 12345678?`,
                );
